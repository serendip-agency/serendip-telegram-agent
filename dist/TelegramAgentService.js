...
class TelegramAgentService {
    constructor(dbService, smsIrService) {
        this.dbService = dbService;
        this.smsIrService = smsIrService;
        Moment.loadPersian({ usePersianDigits: true });
    }
    async start() {
        this.entities = await this.dbService.collection("Entities", false);
        this.businessesCollection = await this.dbService.collection("Businesses", false);
    ...
    async initiateBot(business) {
        const bot = new TelegramBot(business.telegramBotToken, {
            polling: true
        });
        bot.on("message", async (msg, meta) => {
            ...
            if (userQuery.length == 0) {
                user = await this.entities.insertOne(_.extend(user, { _business: business._id, _entity: "telegramUser" }));
            }
            else {
                user = userQuery[0];
            }
    ...
    async handleMessage(bot, business, msg, user) {
        if (user.verified) {
            if (msg.text == "بازگشت")
                return this.showMenu(bot, msg.chat.id);
            ...
            if (["شکایات", "خدمات"].indexOf(msg.text) != -1) {
                ...
                if (list.length == 0) {
                    bot.sendMessage(msg.chat.id, `${msg.text}ی از طرف شما تا کنون ثبت نشده است.`);
                }
                else {
                    bot.sendMessage(msg.chat.id, `لیست ${msg.text}: \n` +
                        list
                            .map(item => {
                            return ("\t\n" +
                                `شناسه: ${item._id}\n` +
                                `تاریخ: ${Moment(item._cdate).format("jYYYY/jMM/jDD HH:mm")} (${Moment(item._cdate).fromNow()})\n` +
                                `متن: ${item.text}\n`);
                        })
                            .join("\n"));
                }
            ...
            if (["شکایات", "خدمات"].indexOf(user.lastMessage.replace("ثبت ", "")) != -1) {
            ...
            if (msg.text == "اطلاعات تماس") {
                bot.sendMessage(msg.chat.id, business.description || "اطلاعات تماس موجود نیست");
            }
            this.showMenu(bot, msg.chat.id);
        }
        else {
            if (msg.text.replace(/\D/g, "").length == 4 && user.verificationCode) {
                if (sUtil.text.replacePersianDigitsWithEnglish(msg.text.replace(/\D/g, "")) == user.verificationCode) {
            ...
            if (msg.text.replace(/\D/g, "").length == 11) {
                bot.sendMessage(msg.chat.id, `کد تایید عضویت به شماره ${sUtil.text.replaceEnglishDigitsWithPersian(msg.text.replace(/\D/g, ""))}
        ارسال شد.
        `);
                bot.sendMessage(msg.chat.id, "لطفا کد ۴ رقمی دریافت شده از طریق پیامک را وارد کنید");
            ...
            bot.sendMessage(msg.chat.id, `سلام ${msg.from.first_name}، \n\n استفاده از ربات تلگرام ${business.title} با تایید شماره موبایل شما امکان‌پذیر خواهد بود.\n\n
        در صورت تمایل به ادامه شماره همراه خود را وارد کنید.`, { reply_markup: { keyboard: [] } });
        }
    }
    showMenu(bot, chatId) {
